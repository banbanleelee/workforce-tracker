...
// Add a new task for the logged-in user (only 'teamMember' can post tasks)
router.post('/', verifyToken('teamMember'), async (req, res) => {
  ...
  try {
    ...
    const { taskId, timeSpent, queue, completed = false } = req.body;
...
router.put('/complete/:taskId', verifyToken('teamMember'), async (req, res) => {
  ...
  const { timeSpent, completed } = req.body; // Get the updated timeSpent and completed status
  ...
  try {
    ...
    res.status(200).json({ message: 'Task completed successfully', task: user.tasks.find(task => task.taskId === taskId) });
  } catch (error) {
